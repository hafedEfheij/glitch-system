...
app.get('/api/admin/students/:id/courses', adminMiddleware, (req, res) => {
  ...
  `, [studentId], (err, student) => {
    ...
    db.all(`
      SELECT cc.*, c.course_code, c.name, c.department_id, c.semester, d.name as department_name
    ...
    `, [studentId], (err, completedCourses) => {
      ...
      db.all(`
        SELECT e.id as enrollment_id, e.*, c.course_code, c.name, c.department_id, c.semester, d.name as department_name,
               g.group_name
        FROM enrollments e
  ...
  db.get('SELECT id, department_id FROM students WHERE user_id = ?', [req.session.user.id], (err, student) => {
    ...
    `, [student.department_id], (err, courses) => {
      ...
      db.all('SELECT course_id FROM completed_courses WHERE student_id = ?', [student.id], (err, completedCourses) => {
        ...
        // Get student's enrolled courses with group information
        db.all(`
          SELECT e.course_id, e.group_id, g.group_name, g.professor_name, g.time_slot
          FROM enrollments e
          LEFT JOIN course_groups g ON e.group_id = g.id
          WHERE e.student_id = ?
        `, [student.id], (err, enrolledCourses) => {
          ...
          // Create a map of enrolled courses to their group info
